#!/usr/bin/env python3
"""
Main script for Balanced Gender Detection
YOLOv8s + DeepFace (Balanced - Best for surveillance)

This version uses YOLOv8s for balanced speed and accuracy, perfect for road surveillance.
"""
...
class BalancedGenderDetector:
    def __init__(self):
        """Initialize the balanced gender detection system."""
        print("🎯 Khởi tạo hệ thống YOLOv8s + DeepFace (CÂN BẰNG TỐI ƯU)...")
        
        # Initialize YOLO detector with small model for balance
        self.detector = YOLODetector(model_path="yolov8s.pt")
        ...
        print("🎯 Dự kiến: 15-20 FPS, độ chính xác: 75-80%")
        print("🎯 Phù hợp cho giám sát đường bộ - bắt kịp 80-95% người đi bộ")
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Display frame
                cv2.imshow("Balanced Gender Detection (YOLOv8s)", processed_frame)
                ...
                if key == ord('q'):
                    ...
                    filename = f"gender_detection_yolov8s_{timestamp}.jpg"
